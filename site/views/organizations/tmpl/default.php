...
<form action="<?php echo JRoute::_('index.php?option=com_wbty_users&view=organizations'); ?>" method="post" name="adminForm" id="adminForm" class="form-inline">
	...
	<h2 class="title centered"><?php echo JText::_('Organizations'); ?></h2>
	<div class="clr"></div>
	...
	<table class="adminlist table table-striped table-bordered">
			...
			<tr>
				...
				<th width="10%">
					...
					<?php if ($canOrder && $saveOrder) :?>
						<?php echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'organizations.saveorder'); ?>
					<?php endif; ?>
				</th>
                <?php } ?>
		...
		<?php foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
                ...
                <td class="center">
                	<div class="btn-group">
                      ...
                      <ul class="dropdown-menu">
                        <?php
                        echo '<li><a href="index.php?option=com_wbty_users&view=organization&layout=default&id='.$item->id.'">View</a></li>';
						if ($canEdit && $this->state->get('filter.state') != -2) {
	                        echo '<li><a href="index.php?option=com_wbty_users&task=organization.edit&id='.$item->id.'">Edit</a></li>';
						}
                        ?>
                ...
                <?php if (0&&isset($this->items[0]->state)) { ?>
				    <td class="center">
					    <?php echo JHtml::_('jgrid.published', $item->state, $i, 'organizations.', $canChange, 'cb'); ?>
				    </td>
                <?php } ?>
                <?php if (0&&isset($this->items[0]->ordering)) { ?>
				    <td class="order">
					    <?php if ($canChange) : ?>
						    <?php if ($saveOrder) :?>
							    <?php if ($listDirn == 'asc') : ?>
								    <span><?php echo $this->pagination->orderUpIcon($i, true, 'organizations.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								    <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'organizations.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							    <?php elseif ($listDirn == 'desc') : ?>
								    <span><?php echo $this->pagination->orderUpIcon($i, true, 'organizations.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								    <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'organizations.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							    <?php endif; ?>
						    <?php endif; ?>
