 ...
 * Wbty_users model.
 */
class Wbty_usersModelfield_type extends WbtyModelAdmin
{
	protected $text_prefix = 'com_wbty_users';
	protected $com_name = 'wbty_users';
	protected $list_name = 'field_types';

	public function getTable($type = 'field_types', $prefix = 'Wbty_usersTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $control='jform', $key=0)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_wbty_users.field_type.'.$control.'.'.$key, 'field_type', array('control' => $control, 'load_data' => $loadData, 'key'=>$key));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_wbty_users.edit.field_type.data', array());
